...
cat "$SQL_FILE" | while IFS=, read -r QUERY_TYPE QUERY_TEXT; do
    ...
    TABLE_NAMES=$(echo "$QUERY_TEXT" | grep -oE '(shoes|shoe_orders|shoe_customers|shoe_clickstream)__[[:alnum:]_]+' | awk -F'__' '{print toupper($1)}' | paste -sd '|')

    echo -n "${QUERY_ID},${QUERY_TYPE},${QUERY_DIST},${QUERY_PART},${TABLE_NAMES},${TABLE_TYPE},${QUERY_INDEX}," | tee -a $OUTPUT_FILE
    
    for i in $(seq 1 $TRIES); do
        # Ejecución de la consulta y captura del tiempo
        RES=$(clickhouse-client --host "${FQDN:=localhost}" --password "${PASSWORD:=}" ${PASSWORD:+--secure} --time --format=Null --query="$QUERY_TEXT" --progress 0 2>&1)
    ...
    # Incrementar el identificador de consulta
    QUERY_ID=$((QUERY_ID + 1))
done
